...
__global__ void layer_256_0(half2 *in, half *F_real, half *F_imag)
{
    ...
    // 使用tensor core,wmma::matrix_a表示计算时为A矩阵，shape of A = M*K，shape of B = K*N
    // D = AB+C
    ...
    // 加载数据，从F_real到Frag_F_real，16表示按照16为一组进行加载
...
{   
    ...
    // block = 4
    extern __shared__ half2 smem_in[];
    int t_block = threadIdx.x + threadIdx.y * blockDim.x; // 0~32 + (0~8)*4
    int block_start = blockIdx.x * 512 * CONT_SIZE;       // (0~4)*512*16 每个block计算的数据量大小为16*512，总共为64*512

    wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> frag_F_real;
    wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> frag_F_imag;
    wmma::load_matrix_sync(frag_F_real, F_real, 16);    // F_real和F_imag是16*16的DFT矩阵
    wmma::load_matrix_sync(frag_F_imag, F_imag, 16);

    int raw_row = threadIdx.x % 4 * 2;  // threadIdx.x = 0~31 raw_row(0,2,4,6))
    int raw_col = threadIdx.x / 4;      //                    raw_col(0~7))
    half2 twiddle_two = W_N_K(512, t_block);

    for (int i = 0; i < 512 * CONT_SIZE; i += NUM_WARP * 16 * 16)   // i< 512*16 i+= 8*16*16 每次循环计算4*512个数据
    {
        wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, half> frag_out_real;
        wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, half> frag_out_imag;
        wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half, wmma::col_major> frag_in_real;
        ...
        int warp_start = i + threadIdx.y * 256;  //（0， 8*16*16, 16*16*16, 32*16*16）(threadIdx.y = num_warp = 8)

        // fragment一共16个元素，每个元素应该是一个向量，里面有16个元素
        for (int j = 0; j < 8; ++j)
        {
            int row = raw_row + j % 4 / 2 * 8 + j % 2;
            int col = raw_col + j / 4 * 8;
            ...
            // if(blockIdx.x == 0 && i==0){
            //     if( threadIdx.y == 0)
            //         printf("j = %d, threadx = %d, raw_row = %d, raw_col = %d, idx = %d\n", j,
            //                                                                                 threadIdx.x, 
            //                                                                                 row, 
            //                                                                                 col, 
            //                                                                                 block_start + warp_start + row + col * 16);
            // }
            
            frag_in_real.x[j] = ele.x;
            frag_in_imag.x[j] = ele.y;
            
            // if(blockIdx.x == 0 && i==0 &&  threadIdx.y == 0){
            //     printf("threadx = %d, %f, idx = %d\n", threadIdx.x, __half2float(ele.x), block_start + warp_start + row + col * 16);
            // }

            // if(blockIdx.x == 0 && i==0){
            //     if( j == 0 && threadIdx.y == 0){
            //         printf("%f, %f, %d, %f\n",  __half2float(frag_in_real.x[8]), __half2float(frag_in_real.x[0]), sizeof(frag_in_real.x[0]), __half2float(ele.x));
            //         // printf("%f\n", __half2float(ele.x));
            //         printf("%d %d\n", sizeof(frag_in_real.x[8]), sizeof(frag_in_real.x[7]));
            //     }
            // }
        }
        // F是左矩阵,是旋转因子，in是右矩阵，是输入数据
        // 每两个warp负责计算一个batch(512个元素)的16点dft
        // if(blockIdx.x == 0 && i==0 &&  threadIdx.y == 0){
        //     printf("threadx = %d, %f, idx = %d\n", threadIdx.x, __half2float(frag_in_real.x[1]));
        // }
        // complex_mul(frag_F_real, frag_F_imag, frag_in_real, frag_in_imag, frag_out_real, frag_out_imag);
    ...
    __syncthreads();
    for (int i = 0; i < 512 * CONT_SIZE; i += NUM_WARP * 32 * 2) // i< 512*16 i+= 512
    {
        int eid = i + t_block;
        half2 ele_0 = smem_in[eid];
        half2 ele_1 = cmul(smem_in[eid + 256], twiddle_two);
...
void tcfftExec(tcfftHandle plan, half *data)
{
    ...
    plan.layer_0[plan.mergings[0]]<<<plan.N * plan.N_batch / n_cont[plan.mergings[0]] / RADIX, 
                                    threads, 
                                    RADIX * sizeof(half2) * n_cont[plan.mergings[0]]>>>((half2 *)data, plan.F_real, plan.F_imag);
    
    // printf("%d\n", plan.N );                    // 512
    // printf("%d\n", plan.N_batch);               // 64
    // printf("%d\n", n_cont[plan.mergings[0]]);   // 16
    // printf("%d\n", RADIX);                      // 512

    
    // batch =64 , length = 512;
    // plan.N * plan.N_batch / n_cont[plan.mergings[0]] / RADIX = 4,
    // threads = {32,8},
    // RADIX * sizeof(half2) * n_cont[plan.mergings[0]] = 32768
