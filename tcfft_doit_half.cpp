...
void setup(double *data, int n, int n_batch)
{
    ...
    for (int j = 0; j < N_batch; ++j)
        for (int i = 0; i < N; ++i)
        {
            in_host[2 * N * j + 2 * i + 0] = __float2half((float)data[2 * N * j + 2 * rev[i] + 0]);
            in_host[2 * N * j + 2 * i + 1] = __float2half((float)data[2 * N * j + 2 * rev[i] + 1]);
        }
...
void finalize(double *result)
{
    ...
    for (int j = 0; j < N_batch; ++j)
        for (int i = 0; i < N; ++i)
        {
            result[0 + i * 2 + 2 * N * j] = __half2float(in_host[2 * i + 0 + 2 * N * j]);
            result[1 + i * 2 + 2 * N * j] = __half2float(in_host[2 * i + 1 + 2 * N * j]);
        }
}
